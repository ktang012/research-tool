...
%% NOTE TO SELF
% WHEN LEARNING INITIAL ENTRY AIM FOR MORE PRECISION THAN RECALL!!!!
% ALSO NEED TO MAKE ANOTHER plotNNs where it's not dictionary dependent
% FIRST ENTRY GENERALIZES TOO MUCH (short length and high threshold)
...
while (newF1 - F1_diff) > prevF1
    ...
    if isempty(dataDict)
        ...
        % from dictionary...
        %newMP = removeMPNeighbors2(mpData,mp,dataDict);
        newMP = mp;
        [candidates,candIndices,d] = selectMPQueries2(mpData,dataDict,newMP,startLen,k);
    ...
    dictEntry = struct('query',bestQuery,...
    ...
    % [dictIndices,newF1,numClassTP,numClassFP,indLengths] = ...
    %    evalDataDict(data,dataDict,bounds,boundLabels,estTotalTPs(1));
    newF1
